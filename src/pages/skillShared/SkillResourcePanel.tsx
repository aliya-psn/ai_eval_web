...
import { FileTreePanel, ResourceFileHeader } from '@/components/file-tree';
...
export function SkillResourcePanel({
  ...
  const handleCopySelectedFile = useCallback(async () => {
    if (!resolved) return;
    const ok = await copyToClipboard(resolved.resource.content ?? '');
    if (ok) {
...
function BinaryFileView({ fileName }: { fileName: string }) {
  return (
    <div className="flex-1 flex flex-col items-center justify-center gap-3 text-muted-foreground">
      ...
      <p className="text-xs">暂不支持预览该文件内容</p>
    </div>
  );
}
  ...
  if (!content) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center gap-3 text-muted-foreground">
        ...
        <p className="text-xs">无法预览该图片文件</p>
      </div>
    );
  }
