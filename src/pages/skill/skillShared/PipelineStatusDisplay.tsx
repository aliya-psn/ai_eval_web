...
const PIPELINE_LABELS: Record<string, string> = {
  pipelineStatus: '审核流水线',
  pipelineInProgress: '审核中',
  pipelineApproved: '已通过',
  pipelineRejected: '审核未通过',
  pipelineNone: '暂无审核流水线',
  pipelineNoMessage: '暂无输出信息',
};
      ...
      <DialogContent className="max-w-3xl max-h-[85vh] flex flex-col gap-0 p-0" onOpenAutoFocus={(e) => e.preventDefault()}>
        ...
        <DialogHeader className="px-6 pt-5 pb-4 border-b shrink-0">
          <div className="flex items-center gap-3">
            <DialogTitle className="text-base">{pipelineLabel('pipelineStatus')}</DialogTitle>
            <Badge className={cn('text-[11px] px-2 py-0.5 h-5 font-medium border-0 gap-1', config.badgeClass)}>
              ...
              {pipelineLabel(config.labelSuffix)}
            </Badge>
              ...
              <div className="px-6 py-4 space-y-4">
                ...
                {selectedNode.message ? (
                  ...
                  <p className="text-xs text-muted-foreground/60 italic">
                    {pipelineLabel('pipelineNoMessage')}
                  </p>
                )}
              </div>
            </div>
...
export function PipelineStatusDisplay({
  pipelineInfo,
  compact = false,
  translationPrefix: _translationPrefix = 'skill',
  onRefresh,
  refreshing,
  ...
  if (!pipelineInfo) {
    ...
    return (
      <p className="text-xs text-muted-foreground py-2">{pipelineLabel('pipelineNone')}</p>
    );
  }
    ...
    <>
      <div className="space-y-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-medium">{pipelineLabel(config.labelSuffix)}</span>
